...
def upsertBill(pconn, user_id, for_date, informed_qty_delta, occupied_qty_delta) :
	cur = pconn.cursor() 
	    ...
	    usql = "update huhula.bill set updated_at=now(), informed_qty=informed_qty+%s, occupied_qty=occupied_qty+%s, credit=cast((cast((informed_qty+%s)*0.1 as double precision)+gift) as double precision), debit=cast((cast((occupied_qty+%s) as double precision)+penalty) as double precision) where user_id='%s' and for_date=cast('%s' as date)" % (informed_qty_delta, occupied_qty_delta, informed_qty_delta, occupied_qty_delta, user_id, for_date)
            logconsole.debug("update bill sql:" + usql)
	    cur.execute(usql)
            if cur.rowcount == 0:
                isql = "INSERT INTO huhula.bill(user_id, for_date, informed_qty, occupied_qty, credit, debit) values('%s',cast('%s' as date),%s,%s,cast(%s*0.1 as double precision),cast(%s as double precision))" % (user_id, for_date, informed_qty_delta, occupied_qty_delta, informed_qty_delta, occupied_qty_delta)
