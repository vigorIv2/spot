...
def upsertBill(pconn, user_id, for_date, informed_qty_delta, occupied_qty_delta) :
	cur = pconn.cursor() 
	    ...
	    usql = "update huhula.bill set updated_at=now(), informed_qty=informed_qty+%s, occupied_qty=occupied_qty+%s, credit=cast((informed_qty+%s)*0.1 as double precision), debit=cast((occupied_qty+%s) as double precision) where user_id='%s' and for_date=cast('%s' as date)" % (informed_qty_delta, occupied_qty_delta, iinformed_qty_delta, occupied_qty_delta, user_id, for_date)
